...
def ParsePacket(pkt):
    ...
    values = {
        'timestamp': unicode(datetime.now().isoformat()),
        'target': unicode(pkt.addr3),
        'source': unicode(pkt.addr2),
        'SSID': unicode(pkt.getlayer(Dot11ProbeReq).info)
    }
    if pkt.haslayer(Dot11):
...
def SendPacket(pkt, node_name):
    ...
    payload['node_name'] = unicode(node_name)
    #import json
...
def setup_interface(iface):
    ...
    #system("sudo ifconfig %(iface)s down ; sudo iwconfig %(iface)s mode monitor ; sudo ifconfig %(iface)s up" % { 'iface': iface })
    ...
    #system("sudo iwconfig %(iface)s channel %(channel)s" % { 'iface': iface, 'channel': channel })
